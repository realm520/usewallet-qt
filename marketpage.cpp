...
//    ui->tipBtn->setToolTip(QString::fromLocal8Bit("所有成交均会以挂单价成交，优先匹配最高价买单与最低价卖单，多余的花费成为代理收入"));
    ui->tipBtn->setToolTip(tr("All orders will deal at the unit price.The highest priority to match the highest bid and the lowest bid.The extra cost to become proxy income."));
...
void MarketPage::getAssets()
{
    ...
    {
        ui->symbolComboBox1->addItem( Blockchain::getInstance()->assetInfoMap.value(key).symbol);
        ui->symbolComboBox2->addItem( Blockchain::getInstance()->assetInfoMap.value(key).symbol);
    }
...
void MarketPage::updateOrders()
{
    ...
    Blockchain::getInstance()->postRPC( toJsonFormat( "id_blockchain_market_list_bids__" + quoteSymbol + "__" + baseSymbol, "blockchain_market_list_bids",
                                               QStringList() << getAssetName(quoteSymbol) << getAssetName(baseSymbol)  ));

    Blockchain::getInstance()->postRPC( toJsonFormat( "id_blockchain_market_list_asks__" + quoteSymbol + "__" + baseSymbol, "blockchain_market_list_asks",
                                               QStringList() << getAssetName(quoteSymbol) << getAssetName(baseSymbol)  ));
    ...
    ui->buySymbolLabel->setText(tr("Buy ") + baseSymbol);
    ui->sellSymbolLabel->setText(tr("Sell ") + baseSymbol);
...
void MarketPage::updateMyOrders()
{
    ...
    Blockchain::getInstance()->postRPC( toJsonFormat( "id_wallet_market_order_list__" + getCurrentQuoteOrBaseAsset(true) + "__" + getCurrentQuoteOrBaseAsset(false),
...
void MarketPage::updateRecentOrdersHistory()
{
    ...
    Blockchain::getInstance()->postRPC( toJsonFormat( "id_blockchain_market_order_history__" + getCurrentQuoteOrBaseAsset(true) + "__" + getCurrentQuoteOrBaseAsset(false),
...
{    
    ...
    if( id.startsWith("id_wallet_market_cancel_order__"))
    {
        ...
        if( result.startsWith("\"result\""))
        {
            ...
            commonDialog.setText(tr("Withdraw transaction has been sent."));
//            commonDialog.setText(QString::fromLocal8Bit("撤单交易已发出"));
    ...
    if( id == "id_wallet_market_submit_bid" || id == "id_wallet_market_submit_ask")
    {
        ...
        {
            CommonDialog commonDialog(CommonDialog::OkOnly);
            commonDialog.setText(tr("Order transaction has been sent."));
            commonDialog.pop();
        }
...
void MarketPage::showBidOrderList()
{
    ...
    ui->bidOrderTableWidget->horizontalHeaderItem(0)->setText(tr("Unit price") + QString("(%1/%2)").arg(quoteSymbol).arg(baseSymbol));
    ...
    for( int i = 0; i < size; i++)
    {
        ...
        AssetInfo quoteAssetInfo = Blockchain::getInstance()->assetInfoMap.value(info.quoteAssetId);
        AssetInfo baseAssetInfo  = Blockchain::getInstance()->assetInfoMap.value(info.baseAssetId);

        if( info.type == "bid_order")
        {
            ui->bidOrderTableWidget->setItem(i,0,new QTableWidgetItem( QString::number( info.ratio.toDouble() * baseAssetInfo.precision / quoteAssetInfo.precision,'g',15) ));
            ui->bidOrderTableWidget->setItem(i,1,new QTableWidgetItem(QString::number(info.orderBalance / quoteAssetInfo.precision / (info.ratio.toDouble() * baseAssetInfo.precision / quoteAssetInfo.precision),'g',15) ));
            ui->bidOrderTableWidget->setItem(i,2,new QTableWidgetItem(QString::number(info.orderBalance / quoteAssetInfo.precision,'g',15) ));
...
void MarketPage::showAskOrderList()
{
    ...
    ui->askOrderTableWidget->horizontalHeaderItem(0)->setText(tr("Unit price") + QString("(%1/%2)").arg(quoteSymbol).arg(baseSymbol));
    ...
    for( int i = 0; i < size; i++)
    {
        ...
        {
            ui->askOrderTableWidget->setItem(i,0,new QTableWidgetItem(QString::number( info.ratio.toDouble() * baseAssetInfo.precision / quoteAssetInfo.precision,'g',15) ));
            ui->askOrderTableWidget->setItem(i,1,new QTableWidgetItem(QString::number(info.orderBalance / baseAssetInfo.precision,'g',15) ));
            ui->askOrderTableWidget->setItem(i,2,new QTableWidgetItem(QString::number(info.orderBalance / baseAssetInfo.precision * (info.ratio.toDouble() * baseAssetInfo.precision / quoteAssetInfo.precision),'g',15) ));
...
void MarketPage::showMyOrders()
{
    ...
    ui->myOrdersTableWidget->horizontalHeaderItem(3)->setText(tr("Unit price ") + quoteSymbol + "/" + baseSymbol);
    ui->myOrdersTableWidget->horizontalHeaderItem(4)->setText(tr("Num ") + baseSymbol);
    ui->myOrdersTableWidget->horizontalHeaderItem(5)->setText(tr("Amount ") + quoteSymbol);
    ...
    for( int i = 0; i < size; i++)
    {
        ...
        ui->myOrdersTableWidget->setItem(i,3,new QTableWidgetItem(QString::number( info.ratio.toDouble() * baseAssetInfo.precision / quoteAssetInfo.precision,'g',15)));


        if( info.type == "ask_order")
        {
            ui->myOrdersTableWidget->setItem(i,2,new QTableWidgetItem(tr("Sell") ));
            ui->myOrdersTableWidget->item(i,2)->setTextColor(QColor(239,104,60));
            ui->myOrdersTableWidget->setItem(i,4,new QTableWidgetItem(QString::number(info.orderBalance / baseAssetInfo.precision,'g',15) ));
            ui->myOrdersTableWidget->setItem(i,5,new QTableWidgetItem(QString::number(info.orderBalance / baseAssetInfo.precision * ( info.ratio.toDouble() * baseAssetInfo.precision / quoteAssetInfo.precision),'g',15) ));

        }
        else if( info.type == "bid_order")
        {
            ui->myOrdersTableWidget->setItem(i,2,new QTableWidgetItem(tr("Buy") ));
            ui->myOrdersTableWidget->item(i,2)->setTextColor(QColor(113,203,90));
            ui->myOrdersTableWidget->setItem(i,4,new QTableWidgetItem(QString::number(info.orderBalance / quoteAssetInfo.precision / ( info.ratio.toDouble() * baseAssetInfo.precision / quoteAssetInfo.precision),'g',15) ));
            ui->myOrdersTableWidget->setItem(i,5,new QTableWidgetItem(QString::number(info.orderBalance / quoteAssetInfo.precision,'g',15) ));
        }

        ui->myOrdersTableWidget->setItem(i,6,new QTableWidgetItem(tr("withdraw")));
...
void MarketPage::setPrecision()
{
    ...
    int quoteAsset      = Blockchain::getInstance()->getAssetId(quoteSymbol);
    int baseAsset       = Blockchain::getInstance()->getAssetId(baseSymbol);

    int quotePrecison   = QString::number( Blockchain::getInstance()->assetInfoMap.value(quoteAsset).precision, 'g', 15).count() - 1;
    int basePrecison    = QString::number( Blockchain::getInstance()->assetInfoMap.value(baseAsset).precision, 'g', 15).count() - 1;
...
void MarketPage::showRecentOrders()
{
    ...
    ui->recentOrdersTableWidget->horizontalHeaderItem(0)->setText(tr("Buy one ") + quoteSymbol + "/" + baseSymbol);
    ui->recentOrdersTableWidget->horizontalHeaderItem(1)->setText(tr("Sell one ") + quoteSymbol + "/" + baseSymbol);
    ...
    for( int i = 0; i < size; i++)
    {
        ...
        AssetInfo quoteAssetInfo = Blockchain::getInstance()->assetInfoMap.value(history.quoteAssetId);
        AssetInfo baseAssetInfo  = Blockchain::getInstance()->assetInfoMap.value(history.baseAssetId);

        ui->recentOrdersTableWidget->setItem(i,0,new QTableWidgetItem( QString::number( history.bidRatio.toDouble() * baseAssetInfo.precision / quoteAssetInfo.precision,'g',15) ));
        ui->recentOrdersTableWidget->setItem(i,1,new QTableWidgetItem( QString::number( history.askRatio.toDouble() * baseAssetInfo.precision / quoteAssetInfo.precision,'g',15) ));
        ui->recentOrdersTableWidget->setItem(i,2,new QTableWidgetItem( QString::number( history.bidPaidAmount / quoteAssetInfo.precision,'g',15) ));
        ui->recentOrdersTableWidget->setItem(i,3,new QTableWidgetItem( QString::number( history.askPaidAmount / baseAssetInfo.precision,'g',15) ));
...
{
    AssetBalanceMap assetBalanceMap = Blockchain::getInstance()->accountBalanceMap.value(ui->accountComboBox->currentText());
...
{
    int asset1 = Blockchain::getInstance()->getAssetId(ui->symbolComboBox1->currentText());
    int asset2   = Blockchain::getInstance()->getAssetId(ui->symbolComboBox2->currentText());
...
void MarketPage::on_myOrdersTableWidget_cellClicked(int row, int column)
{
    if( column == 6)
    {
        ...
        Blockchain::getInstance()->postRPC( toJsonFormat( "id_wallet_market_cancel_order__" + id, "wallet_market_cancel_order", QStringList() << id ));
...
void MarketPage::on_accountComboBox_currentIndexChanged(const QString &arg1)
{
    ...
    Blockchain::getInstance()->currentAccount = arg1;
    showBalance();
}
...
//        commonDialog.setText(QString::fromLocal8Bit("您还没有创建账户!"));
    ...
    Blockchain::getInstance()->postRPC( toJsonFormat( "id_wallet_market_submit_bid", "wallet_market_submit_bid",
...
void MarketPage::on_askBtn_clicked()
{
    ...
    Blockchain::getInstance()->postRPC( toJsonFormat( "id_wallet_market_submit_ask", "wallet_market_submit_ask",
...
void MarketPage::on_priceLineEdit_textChanged(const QString &arg1)
{
    ...
    int quoteAsset      = Blockchain::getInstance()->getAssetId(quoteSymbol);
//    int quotePrecison   = QString::number( Fry::getInstance()->assetInfoMap.value(quoteAsset).precision, 'g', 15).count() - 1;

//    qDebug() << QString::number(amount,'f',quotePrecison).toDouble();
//    qDebug() << QString::number(QString::number(amount,'f',quotePrecison).toDouble(),'g',15);
    ui->amountLineEdit->setText( QString::number(roundDown(amount,Blockchain::getInstance()->assetInfoMap.value(quoteAsset).precision),'g',15) );
...
//    ui->amountLineEdit->setText( QString::number(QString::number(amount,'f',quotePrecison).toDouble(),'g',15) );
    ui->amountLineEdit->setText( QString::number(roundDown(amount,Blockchain::getInstance()->assetInfoMap.value(quoteAsset).precision),'g',15) );
...
void MarketPage::on_amountLineEdit_textChanged(const QString &arg1)
{
    ...
    int baseAsset      = Blockchain::getInstance()->getAssetId(baseSymbol);
    int basePrecision   = QString::number( Blockchain::getInstance()->assetInfoMap.value(baseAsset).precision, 'g', 15).count() - 1;

    ui->numLineEdit->setText( QString::number( QString::number(num,'f',basePrecision).toDouble(),'g',15));
...
//    ui->amountLineEdit2->setText( QString::number(QString::number(amount,'f',quotePrecison).toDouble(),'g',15) );
    ui->amountLineEdit2->setText( QString::number(roundDown(amount,Blockchain::getInstance()->assetInfoMap.value(quoteAsset).precision),'g',15) );
...
void MarketPage::on_amountLineEdit2_textChanged(const QString &arg1)
{
    ...
    ui->numLineEdit2->setText( QString::number( QString::number(num,'f',basePrecision).toDouble(),'g',15));
