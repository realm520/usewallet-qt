    ...
    ui(new Ui::SelectWalletPathWidget)
{
    ...
    ui->pathLineEdit->setText( Blockchain::getInstance()->appDataPath);
...
void SelectWalletPathWidget::on_okBtn_clicked()
{
    ...
    Blockchain::getInstance()->proc->start(ASSET_NAME".exe",QStringList() << "--data-dir" << ui->pathLineEdit->text()
                                       << "--rpcuser" << "a" << "--rpcpassword" << "b" << "--rpcport" << QString::number( RPC_PORT) << "--server");
    if( Blockchain::getInstance()->proc->waitForStarted())
    {
        mutexForConfigFile.lock();
        Blockchain::getInstance()->configFile->setValue("/settings/chainPath", ui->pathLineEdit->text());
        mutexForConfigFile.unlock();
        Blockchain::getInstance()->appDataPath = ui->pathLineEdit->text();
        emit enter();
    }
    else
    {
