...
//qDebug() << m_id << cmd;
    ...
    do
    {
        if( !socket->waitForReadyRead(10000) )
        {
            ...
            Blockchain::getInstance()->rpcReceivedOrNotMapSetValue(cmd, true);
...
//    qDebug() << m_id << receive;
    ...
    Blockchain::getInstance()->updateJsonDataMap(rpcId, result);
...
void WorkerThread::run()
{
    ...
    socket->connectToHost(QHostAddress("127.0.0.1"),Blockchain::getInstance()->currentPort);
